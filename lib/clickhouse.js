...
export function initClickHouseClient(config) {
  ...
  // Only apply settings if explicitly enabled or for specific users
  // Avoid default settings that might fail in read-only environments
  if (config.username !== 'play' && config.username !== 'demo') {
    clientConfig.clickhouse_settings = {
